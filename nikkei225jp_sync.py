...
株探/JPX公式データを補完する第三のデータ源。日経225指数ベースのPER/PBR、
騰落レシオ、信用残高、主体別売買動向、空売り比率、NT倍率、裁定買い残、
先物週次建玉、恐怖指数を提供する。kabutan.jpには一切アクセスしない。
...
素のHTTP GET(requests)では空のテーブルしか取得できない(実運用環境での実行で確認済み)。
そのためPlaywright(ヘッドレスブラウザ)でJSを実行させてから取得する。
セットアップ時に一度 `playwright install chromium` が別途必要。
"""
import re
...
def sync_shutai(conn, session: Nikkei225jpSession | None = None) -> dict:
    try:
    ...
    for rec in records:
        db.upsert_record(conn, "nikkei225jp_investor_trends", ("date",), rec)
    conn.commit()
...
def sync_sinyou(conn, session: Nikkei225jpSession | None = None) -> dict:
    try:
    ...
    for rec in records:
        db.upsert_record(conn, "nikkei_margin_records", ("date",), rec)
    conn.commit()
...
def sync_karauri(conn, session: Nikkei225jpSession | None = None) -> dict:
    try:
    ...
    for rec in records:
        db.upsert_record(conn, "nikkei225jp_short_selling", ("date",), rec)
    conn.commit()
...
def sync_touraku(conn, session: Nikkei225jpSession | None = None) -> dict:
    try:
    ...
    for rec in records:
        db.upsert_record(conn, "nikkei_touraku_records", ("date",), rec)
    conn.commit()
...
def sync_nt(conn, session: Nikkei225jpSession | None = None) -> dict:
    try:
    ...
    for rec in records:
        db.upsert_record(conn, "nikkei225jp_nt_ratio", ("date",), rec)
    conn.commit()
...
def sync_saitei(conn, session: Nikkei225jpSession | None = None) -> dict:
    try:
    ...
    for rec in records:
        db.upsert_record(conn, "nikkei225jp_arbitrage", ("date",), rec)
    conn.commit()
...
def sync_futures(conn, session: Nikkei225jpSession | None = None) -> dict:
    try:
    ...
    for rec in records:
        db.upsert_record(conn, "nikkei225jp_futures_broker", ("date",), rec)
    conn.commit()
...
def sync_vix(conn, session: Nikkei225jpSession | None = None) -> dict:
    try:
    ...
    for rec in records:
        db.upsert_record(conn, "nikkei225jp_fear_index", ("date",), rec)
    conn.commit()
...
def sync_per(conn, session: Nikkei225jpSession | None = None) -> dict:
    try:
    ...
    for rec in records:
        db.upsert_record(conn, "nikkei_per_records", ("date",), rec)
    conn.commit()
...
def sync_all(conn, session: Nikkei225jpSession | None = None) -> dict:
    ...
    try:
        return {
            "per": sync_per(conn, session),
            "shutai": sync_shutai(conn, session),
            "sinyou": sync_sinyou(conn, session),
            "karauri": sync_karauri(conn, session),
            "touraku": sync_touraku(conn, session),
            "nt": sync_nt(conn, session),
            "saitei": sync_saitei(conn, session),
            "futures": sync_futures(conn, session),
            "vix": sync_vix(conn, session),
        }
    finally:
        if own_session:
            session.close()
